...
st.set_page_config(page_title="Scan Pépites Isolées", layout="wide")
...
st.title("🚀 Détecteur de Bâtiments Isolés")

# --- BARRE LATÉRALE (INTERFACE) ---
with st.sidebar:
    st.header("Paramètres du Scan")
    commune_in = st.text_input("Commune centre :", value="Turenne, France")
    ...
    st.markdown("---")
    lancer_scan = st.button("🚀 LANCER LE SCAN FINAL")
        ...
        with st.spinner("📡 Analyse géographique en cours..."):
            ...
            if not candidates.empty:
                ...
                if res.empty:
                    ...
                    # Limites de la zone
                    folium.GeoJson(
                        secteur.to_crs(epsg=4326), 
                        style_function=lambda x: {'fillColor': 'transparent', 'color': 'cyan', 'weight': 2}
                    ).add_to(m)
